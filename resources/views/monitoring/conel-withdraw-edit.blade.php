...
@section('title', 'Edit Conel Withdrawal')
@section('page-title', 'Edit Conel Withdrawal')

@section('header-actions')
    <a href="{{ route('monitoring.conel') }}" class="btn btn-secondary">Cancel</a>
@endsection

@section('content')
    <div class="panel p-6">
        <form method="POST" action="{{ route('monitoring.conel.withdraw.update', $transaction) }}" class="space-y-6">
            @csrf
            @method('PUT')
